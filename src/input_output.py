...
def moments(S, L):
    ...
    Returns (S_12, S_14, NdN, NN, comm) where
        S_12                                    (transduction amplitude),
        S_14                                    (two-mode squeezing amplitude),
        NdN  = <N† N>                           (added photon number),
        NN   = <N N>                            (anomalous noise moment),
        comm = [N, N†] = 1 - |S_12|² + |S_14|²  (noise commutator).
    ...
    # Second moments of N over the vacuum ancilla ports (optical + internal loss).
    NdN = np.abs(S[0, 2])**2 + np.abs(L[0, 2])**2 + np.abs(L[0, 3])**2
    NN  = S[0, 0]*S[0, 2] + L[0, 0]*L[0, 2] + L[0, 1]*L[0, 3]

    comm = 1 - np.abs(S_12)**2 + np.abs(S_14)**2
    return S_12, S_14, NdN, NN, comm
